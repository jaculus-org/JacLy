...
import { addShadowNumber } from '@/workspace/shadows/shadow-blocks';
import { registerTypedVariableCategory, type TypedVarConfig } from './utils';
  ...
  customContextMenu(
    ...
    if (!this.isInFlyout) {
      ...
      options.push({
        ...
        text: (Blockly.Msg.VARIABLES_GET_CREATE_SET || 'Create "set %1"').replace(
          '%1',
          varField.getText(),
        ),
...
Blockly.Blocks[BLOCK_SET] = {
  ...
  customContextMenu(
    ...
    if (!this.isInFlyout) {
      ...
      options.push({
        enabled: this.workspace.remainingCapacity() > 0,
        text: (Blockly.Msg.VARIABLES_SET_CREATE_GET || 'Create "get %1"').replace(
          '%1',
          varField.getText(),
        ),
        callback: Blockly.ContextMenu.callbackFactory(this, {
...
jsg.init = function (workspace: Blockly.Workspace) {
  ...
  for (let i = 0; i < devVarList.length; i++) {
    defvars.push(this.nameDB_?.getName(devVarList[i], Blockly.Names.NameType.DEVELOPER_VARIABLE));
  }
...
jsg.forBlock[BLOCK_GET] = (block: Blockly.Block) => {
  const code = jsg.getVariableName(block.getFieldValue('VAR'));
  return [code, Order.ATOMIC];
};

jsg.forBlock[BLOCK_SET] = (block: Blockly.Block) => {
...
export function registerVariableCategoryCallback(workspace: Blockly.WorkspaceSvg): void {
