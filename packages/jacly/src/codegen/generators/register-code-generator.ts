...
import { getPlaceholderValue, replacePlaceholders } from '@/codegen/placeholders/placeholder-utils';
import type { JaclyBlock, JaclyBlockKindBlock } from '@/schema';
...
function selectConditionalCode(
  ...
  if (block.codeConditionals && block.codeConditionals.length > 0) {
    for (const conditional of block.codeConditionals) {
      if (evaluateConditions(state, conditional.condition, block.args0, codeBlock, generator)) {
