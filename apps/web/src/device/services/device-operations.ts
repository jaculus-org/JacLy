...
export async function version(device: JacDevice): Promise<{ esp32: string; dcore: string } | null> {
  try {
...
export async function addWifiNetwork(device: JacDevice, ssid: string, password: string) {
  try {
    await withLockedDevice(device, async (dev) => {
